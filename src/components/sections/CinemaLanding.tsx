...
const CORRE = [0.26, 0.93] as const; // a página corre lá dentro
...
/* Portátil: tampa que abre com o scroll, aro de alumínio, bisel preto, base
   mais larga com a saliência da dobradiça. Tudo CSS, sem marca nenhuma. */
function MacBook({
  url,
  conteudo,
  rotacao,
  ...
  return (
    <div className="relative w-full" style={{ perspective: 1800 }}>
      <motion.div
        ...
        className="rounded-[9px] bg-gradient-to-b from-[#9ba1aa] via-[#6f757e] to-[#43474e] p-[1px] shadow-[0_60px_130px_-45px_rgba(0,102,255,.55)] md:rounded-[16px] md:p-[2px]"
      >
        <div className="relative rounded-[8px] bg-[#08080b] p-[3px] md:rounded-[14px] md:p-[9px]">
          {/* Câmara */}
          <span className="absolute left-1/2 top-[1px] h-[2px] w-[2px] -translate-x-1/2 rounded-full bg-[#26292f] md:top-[3.5px] md:h-[3px] md:w-[3px]" />
          <motion.div style={ecraOpacity ? { opacity: ecraOpacity } : undefined} className="aspect-[16/10] w-full">
            <div className="flex h-full flex-col overflow-hidden rounded-[3px] bg-[#0b0b13] md:rounded-[6px]">
          ...
          {capaOpacity && (
            <motion.div
              ...
              className="pointer-events-none absolute inset-0 rounded-[8px] bg-[linear-gradient(168deg,#c3c9d1_0%,#9aa0a9_38%,#767c85_68%,#a8aeb7_100%)] md:rounded-[14px]"
            />
          )}
        </div>
      </motion.div>

      {/* Base: mais larga que a tampa, com a mossa da dobradiça ao centro. */}
      <div className="relative left-1/2 h-[4px] w-[107%] -translate-x-1/2 rounded-b-[3px] bg-[linear-gradient(180deg,#e2e6eb_0%,#b6bcc4_45%,#7d838c_100%)] shadow-[0_16px_28px_-12px_rgba(0,0,0,.95)] md:h-[10px] md:rounded-b-[8px]">
        <span className="absolute left-1/2 top-0 h-[2px] w-[11%] -translate-x-1/2 rounded-b-full bg-[#868c95] md:h-[4px]" />
      </div>
    </div>
  );
}
